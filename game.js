...
function renderActions() {
    ...
    // 🔥 직업 전용 스킬 버튼 렌더링 (장비를 샀을 때만 보임!)
    if (player.unlockedSkill) {
        const skillBtn = document.createElement('button');
        skillBtn.innerText = `🔥 ${player.unlockedSkill}`;
        skillBtn.style.background = '#9b59b6'; // 보라색 에픽 스킬 느낌
...
function useAction(type) {
    if (type === '공격') {
        ...
        if (Math.random() * 100 < 70) { shieldedTurns = 2; writeLog(`[성공] 2턴 동안 적의 데미지를 1회 무효화합니다!`); } 
...
function winBattle() {
    ...
    writeLog(`[승리] ${gain}G 획득 및 체력 회복.`); floor++; spawnEnemy();
}
...
function renderShopItems() {
    ...
    currentShopItems = [{ name: "치유 포션", type: "potion", value: 80, price: 40, rarity: "common", desc: "체력을 80 즉시 회복합니다." }];
    ...
    currentShopItems.forEach((it, idx) => {
        ...
        let color = '#ccc';
        if (it.rarity === 'legendary') color = '#e74c3c'; // 전설 등급 빨간색!
...
function buyItem(event, idx) {
    ...
    updateUi(); renderActions(); // 궁극기 버튼을 화면에 바로 띄우기 위해 다시 렌더링!
}
